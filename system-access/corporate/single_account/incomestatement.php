                        ...
                        <table class="mws-table">
							...
							foreach($ld_sales_discount as $row_sales_discount)
							{
								...
								$inid_sales_discount=$row_sales_discount->id; 
									$getdutotal_sales_discount=$obj->SelectAllByID_Multiple("account_module_ladger",array("ladger_id"=>$inid_sales_discount));
							...
							foreach($ld_sales_return as $row_sales_return)
							{
								...
								$inid_sales_return=$row_sales_return->id; 
									$getdutotal_sales_return=$obj->SelectAllByID_Multiple("account_module_ladger",array("ladger_id"=>$inid_sales_return));
							...
							foreach($sql_o_i as $row_o_i)
							{
								...
								foreach($ld_other_income as $row_other_income)
								{
									...
									$inid_other_income=$row_other_income->id; 
										$getdutotal_other_income=$obj->SelectAllByID_Multiple("account_module_ladger",array("ladger_id"=>$inid_other_income));
							...
							foreach($ld_begining_inventory as $row_begining_inventory)
							{
								...
								$inid_begining_inventory=$row_begining_inventory->id; 
									$getdutotal_begining_inventory=$obj->SelectAllByID_Multiple_Rep_Date("account_module_ladger",array("ladger_id"=>$inid_begining_inventory),"ladger_date",$to,$from);
							...
							foreach($ld_ending_inventory as $row_ending_inventory)
							{
								...
								$inid_ending_inventory=$row_ending_inventory->id; 
									$getdutotal_ending_inventory=$obj->SelectAllByID_Multiple_Rep_Date("account_module_ladger",array("ladger_id"=>$inid_ending_inventory),"ladger_date",$endto,$endfrom);
...
<!--                             <tr><th valign="middle" align="left"> Direct Expense</th>
							...
							foreach($inventorypurchase as $row_purchase_inventory)
							{
								...
								$inid_purchase_inventory=$row_purchase_inventory->id; 
									$getdutotal_purchase_inventory=$obj->SelectAllByID_Multiple("account_module_ladger",array("ladger_id"=>$inid_ending_inventory));
							...
							foreach($ld_purchase_discount as $row_purchase_discount)
							{
								...
								$inid_purchase_discount=$row_purchase_discount->id; 
									$getdutotal_purchase_discount=$obj->SelectAllByID_Multiple("account_module_ladger",array("ladger_id"=>$inid_purchase_discount));
							...
							foreach($ld_purchase_return as $row_purchase_return)
							{
								...
								$inid_purchase_return=$row_purchase_return->id; 
									$getdutotal_purchase_return=$obj->SelectAllByID_Multiple("account_module_ladger",array("ladger_id"=>$inid_purchase_return));
                            ...
                            <tr>
								...
								$year=date('Y');
								$date=date('Y-m-d');
								$chkincomestatement=$obj->exists_multiple("account_module_income_statement",array("year"=>$year));
								if($chkincomestatement!=0)
								{
									$obj->update("income_statement",array("year"=>$year,"amount"=>$netprofit,"totalincome"=>$tincome,"directexpense"=>$cogs,"grossprofit"=>$gross_profit,"date"=>$date));
								
								}
								else
								{
									$obj->insert("income_statement",array("year"=>$year,"amount"=>$netprofit,"totalincome"=>$tincome,"directexpense"=>$cogs,"grossprofit"=>$gross_profit,"date"=>$date));
