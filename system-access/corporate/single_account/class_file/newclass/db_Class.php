...
class db_class {
    public function open() {
        @$con = mysqli_connect("localhost", "root", "", "amsit_pk_accounts");
		//@$con = mysqli_connect("localhost", "ptdxrklv_pkgroup", "amsit", "ptdxrklv_pkgroup");
        return $con;
    }
   ...
   function exists_range($val,$min,$max,$val2)
    ...
    {
        $errmsg_arr[]="<span style='font-family:Tahoma, Geneva, sans-serif;font-size:16px;color:#F00;padding-left:25px;padding-top:5px;margin-top:5px;'>".$msg."</span>";
        $errflag = true;
        if ($errflag) {
    ...
    {
        $errmsg_arr[]="<span style='font-family:Tahoma, Geneva, sans-serif;font-size:16px;color:#090;padding-left:25px;padding-top:5px;margin-top:5px;'>".$msg."</span>";
        $errflag = true;
        if ($errflag) 
        {
   ...
   $count = 0; $fields = ''; $con = $obj_pos->open();
    $query = "SELECT * FROM `$object` t where t.date>='$from' and t.date<='$to'"; 
	$result = mysqli_query($con, $query); if ($result) {
		 $count = mysqli_num_rows($result); if($status==1)
		  { 
		  $objects=array(); while($rows = $result->fetch_object()) {
			   $objects[] = $rows; } $obj_pos->close($con); 
			   return $objects; 
			   } else { 
			   $obj_pos->close($con); 
			   return $count; } 
