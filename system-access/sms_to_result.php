...
$table = "ams_result_wise_sms";
 if (isset($_POST['save']))
 {
	 
    foreach($_POST['admission'] as $index=>$nn):
			if($obj->exists_multiple($table,array("admission"=>$nn,"class_id"=>$_POST['class'],"pass_fail"=>$_POST['result'][$index],"number"=>$_POST['number'][$index],"school_id"=>$school))==0)
			{
        		$obj->insert($table,array("admission"=>$nn,"class_id"=>$_POST['class'],"pass_fail"=>$_POST['result'][$index],"number"=>$_POST['number'][$index],"date"=>date('Y-m-d'),"satuts"=>1,"school_id"=>$school));
			}
	endforeach;
