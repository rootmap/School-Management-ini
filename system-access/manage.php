...
if(isset($_POST['save']))
{
    ...
    $obj->insert($table,array("group_id"=>$_POST['group'],"id_number"=>$_POST['number'],"receipt"=>$_POST['receipt'],"school_id"=>$school,"date"=>date('Y-m-d'),"school_id"=>$school,"status"=>1));
		...
		if($count!=0)
		{
			echo $obj->Success("Data Saved Successfully",$obj->filename()."?id=".$id."&school=".$school);	
		}
		else
		{
...
<tr>
        ...
        <td class="center"><a href="#" onclick="managememnt('<?php echo $row->id;?>')" type="radio" name="id_number" id="id_number"><img src="images/Calculator-icon.gif" /></a></td>
...
<?php $s++; } ?>
                                    ...
                                    <div id="dynamic1" style="width:100%; margin-top:30px; float:right;"></div>
                            </div>
                            <div class="clear"></div>
                        </div>
