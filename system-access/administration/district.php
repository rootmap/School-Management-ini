...
if(isset($_POST['save']))
{
	if(!empty($_POST['name']))
	{
		$array=array("name"=>$_POST['name'],"division_id"=>$_POST['division_id'],"country_id"=>1,"date"=>date('Y-m-d'),"status"=>1);
		if($obj->insert($table,$array)==1)
		{
			echo $obj->Success("District Successfully Saved",$obj->filename());	
		}
		else
		{
			echo $obj->Error("District Save Failed, Please Try Again",$obj->filename());	
		}
	}
	else
	{
		echo $obj->Error("District Field Is Empty, Please Try Again",$obj->filename());	
	}
}
...
if(isset($_POST['update']))
{
	if(!empty($_POST['name']))
	{
		$array=array("id"=>$_POST['id'],"name"=>$_POST['name'],"division_id"=>$_POST['division_id'],"date"=>date('Y-m-d'),"status"=>1);
		if($obj->update($table,$array)==1)
		{
			echo $obj->Success("District Successfully Updated",$obj->filename());	
		}
		else
		{
			echo $obj->Error("District Update Failed, Please Try Again",$obj->filename());	
		}
	}
	else
	{
...
if(isset($_GET['del']))
{
		...
		{
			echo $obj->Success("District Successfully Deleted",$obj->filename());	
		}
		else
		{
			echo $obj->Error("District Deletion Failed, Please Try Again",$obj->filename());	
		}
}
?>
...
<div id="body_all" class="padding_top_5 padding_butttom_5">
   ...
   <div class="backgroung_3 width_100 padding_bottom10 padding_top10 text_aling_center font_2 color_w" style="font-weight:bolder; font-size:22px;">
     <?php if(isset($_GET['edit'])){ ?> Update District Name <?php }else{ ?> New District Name <?php } ?>
                 </div>
                 <div class="width_100">
                  <style type="text/css">
                   ...
                   <table id="dd" class="width_100 font_2 color_b" cellpadding="0" cellspacing="1" border="0">
                     ...
                     <tr class="backgroung_2">
                       ...
                       <select  class="borber2 padding_5" name="division_id">
                            ...
                            foreach ($div2 as $rw2):
							?>
                            <option <?php if($div_id==$rw2->id){ ?> selected="selected" <?php } ?> value="<?php echo $rw2->id; ?>"><?php echo $rw2->name; ?></option>
                            <?php endforeach; ?>
                       </select>
                       </td>
     ...
     <h3>District List</h3>
          ...
          <?php 
			...
			$chkdiv=$obj->exists_multiple($table,array("division_id"=>$row2->id));
			if($chkdiv!=0)
			{
		  ?> 
          <thead class="backgroung_3 color_w">
            <th class="padding_5" colspan="4" align="left" valign="middle" height="30"><?php echo $s2." - : ".$row2->name; ?></th>
          </thead>      
          ...
          <?php 
		    $data=$obj->SelectAllByID_Multiple($table,array("division_id"=>$row2->id));
			$s=1;
			if(!empty($data))
			foreach ($data as $row):
		  ?>
