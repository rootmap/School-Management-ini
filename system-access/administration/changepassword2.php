...
if(!isset($_SESSION['SESS_AMSIT_EIIN_KEY']))
{
  $obj->Error("Please Login.","eiin_number_setting.php");	
}

$master_input=$_SESSION['SESS_AMSIT_EIIN_KEY'];
$table="ams_admin_eiin_chk";
if(isset($_POST['update']))
{
	if(!empty($_POST['username']) && !empty($_POST['password']) && !empty($_POST['oldpassword']))
	{
		$expassword=$obj->SelectAllByVal($table,"id",$master_input,"password");
		...
		else
		{
			...
			{
				echo $obj->Success("Your Master Login Info Successfully Updated",$obj->filename());	
			}
			else
			{
				echo $obj->Error("Master Login Info Update Failed, Please Try Again",$obj->filename());	
			}
		}
	}
	else
...
<title>Change Password | Master User | ADMINISTRATION</title>
                  ...
                  <style type="text/css">
                   ...
                   <table id="dd" class="width_100 font_2 color_b" cellpadding="0" cellspacing="1" border="0">
                     ...
                     <tr class="backgroung_2">
					   ...
					   $username=$obj->SelectAllByVal($table,"id",$master_input,"name");					   
					   ?></td>
