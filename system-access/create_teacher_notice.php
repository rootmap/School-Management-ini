...
if(isset($_POST['save']))
{
    ...
    if($obj->exists_Multiple($table,array("name"=>$_POST['name'],"details"=>$_POST['text'],"school_id"=>$school,"status"=>2))==0 
	&& !empty($_POST['name']) 
	&& !empty($_POST['text']))
    {
		$uf=$obj->pdf("word",time(),"pdf");
        
       if( $obj->insert($table,array("name"=>$_POST['name'],"word"=>$uf,"details"=>$_POST['text'],"school_id"=>$school,"date"=>date('Y-m-d'),"status"=>2))==1)
	
	{
		echo $obj->Success("Teacher Notice Data Saved Successfully( ".$count." )",$obj->filename());
	}
	else
	{
		echo $obj->Error("Teacher Notice Data Saved Failed( ".$count." )",$obj->filename());	
	}
  }
     else
	{
...
if(isset($_POST['update']))
{
	...
	if($obj->update($table,$array)==1)
      ...
      if($_POST['name'])
	  {
		echo $obj->Success("Teacher Notice  Update Successfully",$obj->filename());	
	  }
	     else
	  {
		echo $obj->Error("Teacher Notice  Update Failed",$obj->filename());	
	  }
}

if (@$_GET['action'] == 'delete') {
    if ($obj->delete($table, array("id" => $_GET['id'])) == 1) {
        $error_data[] = "<div class='sucess'>Teacher Notice  Successfully Deleted</div>";
        $error_flag = TRUE;
        if ($error_flag) {
        ...
        $error_data[] = "<div class='error_msg'>Teacher Notice  Delete Failed</div>";
        $error_flag = TRUE;
        if ($error_flag) {
            ...
            <div class="st_detail_body">
                <div class="st_detail_topbar">
                    <a href="#">
                        ...
                        <div class="topbar_small_div_text2 left height-30"><h2>Create Teacher Notice</h2></div>
                    </a>
                ...
                <div style="clear:both; ">
                    ...
                    <div class="admission_div shadow_inner" style="margin-top: 20px; height:auto;">
                        <div class="st_group_text_div height-40" style="border-bottom: 5px #CCCCCC ridge;">
                            <h2 class="margin_left30"><left> Create New Teacher Notice </left></h2>
                        </div>
...
<div class="clear"></div>
                    ...
                    <div class="admission_div" style="margin-top: 20px; height:auto;">
                        <div class="st_group_text_div2 height-40" style="border-bottom: 5px #CCCCCC ridge;">
                            <h2 class="margin_left30"><left>Teacher Notice List</left></h2>
                        </div>
                        <div class="admission_div_page auto-position shadow_inner" style="border:none; padding:10px;">
                            ...
                            <div id="dynamic">
                                <table cellpadding="0" cellspacing="1" border="0" class="display" id="example">
                                    ...
                                    <tbody>
                                        <?php
										$aary=array("school_id"=>$school,"status"=>2);
