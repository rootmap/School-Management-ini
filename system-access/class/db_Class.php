...
class db_class {
    ...
    function GetCstomRes($object, $fetch, $fetch1, $stvalue, $f1, $v1, $f2, $v2, $let) {
        $link = $this->open();
        $sql = "SELECT `$let` FROM `$object` WHERE `$f1`='$v1' AND `$f2`='$v2' AND $stvalue BETWEEN `$fetch` AND `$fetch1`";
        //$sql="SELECT `$let` FROM `$object` WHERE `$f1`='$v1' AND `$f2`='$v2' AND $stvalue' between `$fetch` AND `$fetch1`";
        $result = mysqli_query($link, $sql);
        if ($result) {
            $row = mysqli_fetch_array($result);
    ...
    function GetCstomResLetter($object, $fetch, $stvalue, $f1, $v1, $f2, $v2, $let) {
        $link = $this->open();
        $sql = "SELECT `$let` FROM `$object` WHERE `$f1`='$v1' AND `$f2`='$v2' AND  `$fetch`>='$stvalue' or `$fetch`<='$stvalue'";
        //$sql="SELECT `$let` FROM `$object` WHERE `$f1`='$v1' AND `$f2`='$v2' AND $stvalue' between `$fetch` AND `$fetch1`";
        $result = mysqli_query($link, $sql);
        if ($result) {
            $row = mysqli_fetch_array($result);
    ...
    function baseUrl($suffix = '') {
        ...
        if ($_SERVER['HTTP_HOST'] == "localhost") {
            $web_root = $protocol . $_SERVER['HTTP_HOST'] . "/" . "natoparabdm/system-access/";
        } else {
            $web_root = $protocol . $_SERVER['HTTP_HOST'] . "/" . "system-access/";
        }
    ...
    function empreq_status($st) {
        if ($st == 1) {
            $power = "Store Requisation";
        } elseif ($st == 2) {
            $power = "Vehicle Request";
        } elseif ($st == 3) {
            $power = "Meeting Reservation Request";
        } elseif ($st == 4) {
            $power = "Leave Request";
        }
        return $power;
